...
# Paths
# -------------------------
COMBINED_CSV <- "C://Users//admin//Desktop//Coding//exon_shadow_exitron//shadow_sequence_content//plot//inter_data//boxplot_stats.csv"
OUTPUT_PLOT <- "C://Users//admin//Desktop//Coding//exon_shadow_exitron//shadow_sequence_content//plot//GC_content_sidebyside.pdf"
...
p <- ggplot(df, aes(x = xpos, group = interaction(species, region), fill = region)) +
  ...
  geom_text(aes(y = median, label = paste0(round(median, 2))),
            color = "darkgreen", vjust = 1.2, size = 2.5) +
  geom_text(aes(y = mean, label = paste0(round(mean, 2))),
            color = "blue", vjust = -0.6, size = 2.5) +
  ...
  labs(
    title = "GC Content Across Species (Upstream vs Downstream)",
    x = "Species",
    y = "GC Content (%)",
    fill = "Region"
  ) +
