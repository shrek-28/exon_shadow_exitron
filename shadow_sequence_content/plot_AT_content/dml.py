...
BASE_OUTPUT_DIR = r"C:\Users\admin\Desktop\Coding\exon_shadow_exitron\shadow_sequence_content\plot_AT_content"
...
def summarize_upstream_downstream():
    ...
    for sp in SPECIES_ORDER:
        for region in ["upstream", "downstream"]:
            ...
            df = pd.read_csv(fpath, usecols=["AT_percent"])
            
            stats = {
                "species": SPECIES_LABELS[sp],
                "region": region.capitalize(),  # For plot fill/color
                "min": df["AT_percent"].min(),
                "q1": df["AT_percent"].quantile(0.25),
                "median": df["AT_percent"].median(),
                "q3": df["AT_percent"].quantile(0.75),
                "max": df["AT_percent"].max(),
                "mean": df["AT_percent"].mean(),
                "count": len(df['AT_percent'])
            }
