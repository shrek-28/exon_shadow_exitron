...
# Paths
# -------------------------
COMBINED_TSV <- "C://Users//admin//Desktop//Coding//exon_shadow_exitron//fourier_processing//plot_metric//metrics_statistics.tsv"
...
df$seq_type <- factor(
  df$seq_type,
  levels = c("upstream", "downstream", "SCFR", "exitron")
)
...
p <- ggplot(df, aes(x = xpos, group = interaction(species, seq_type), fill = seq_type)) +
  ...
  # Labels
  geom_text(aes(y = .data$median, label = round(.data$median, 3)),
            color = "darkgreen", vjust = 1.2, size = 2.4) +
  geom_text(aes(y = .data$mean, label = round(.data$mean, 3)),
            color = "blue", vjust = -0.7, size = 2.4) +
  geom_text(aes(y = .data$min, label = round(.data$min, 3)),
            color = "red", vjust = 1.6, size = 2.4) +
  geom_text(aes(y = .data$max, label = round(.data$max, 3)),
...
ggsave("C://Users//admin//Desktop//Coding//exon_shadow_exitron//fourier_processing//metric_distribution.pdf", width=14, height=6, dpi=300)