...
for file in files:
    ...
    df = df[['gene_tx_count', 'downstream_len_in_scfr']]
    ...
    # -----------------------------
    groups = [group['downstream_len_in_scfr'].values for _, group in df.groupby('gene_tx_count')]
...
results_df.to_csv("statistical_analysis/shadow_length_vs_gene_tx_count/results/downstream_exon_shadow_vs_gene_tx_count.csv", index=False)
