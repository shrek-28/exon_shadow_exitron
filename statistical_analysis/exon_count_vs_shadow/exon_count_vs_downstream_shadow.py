...
for file in files:
    ...
    df = df[['exon_count', 'downstream_len_in_scfr']]

    print(species)
    print("df:", len(df))

    print(df['downstream_len_in_scfr'].value_counts())
    df = df[df['downstream_len_in_scfr'] != 0]
    print("after dropping zeros:", len(df))
  
    n_total = len(df)

    # Skip species for tests only if <2 exon_count groups
    if df['exon_count'].nunique() < 2:
    ...
    # -----------------------------
    # Group data by exon_count
    # -----------------------------
    groups = [group['downstream_len_in_scfr'].values for _, group in df.groupby('exon_count')]
...
results_df.to_csv("statistical_analysis/exon_count_vs_shadow/results/downstream_exon_shadow_vs_exon_count.csv", index=False)
