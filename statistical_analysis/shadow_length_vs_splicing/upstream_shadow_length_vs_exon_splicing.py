...
for file in files:
    ...
    df = df[['first_exon_splicing', 'upstream_len_in_scfr']]
    ...
    # Group data
    groups = {
        cat: df[df['first_exon_splicing'] == cat]['upstream_len_in_scfr'].values
...
summary_df.to_csv(
    "statistical_analysis/shadow_length_vs_splicing/results/first_exon_splicing_upstream_shadow_summary.csv", index=False
)
posthoc_df.to_csv(
    "statistical_analysis/shadow_length_vs_splicing/results/first_exon_splicing_upstream_posthoc_tests.csv", index=False
)
