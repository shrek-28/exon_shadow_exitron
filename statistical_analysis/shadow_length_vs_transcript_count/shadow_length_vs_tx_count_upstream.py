...
for file in files:
    ...
    df = df[['first_exon_tx_count', 'upstream_len_in_scfr']]

    print(species)
    print("df:", len(df))

    print(df['upstream_len_in_scfr'].value_counts())
    df = df[df['upstream_len_in_scfr'] != 0]
    print("after dropping zeros:", len(df))
  
    n_total = len(df)

    # Skip species for tests only if <2 first_exon_tx_count groups
    if df['first_exon_tx_count'].nunique() < 2:
    ...
    # -----------------------------
    # Group data by first_exon_tx_count
    # -----------------------------
    groups = [group['upstream_len_in_scfr'].values for _, group in df.groupby('first_exon_tx_count')]
...
results_df.to_csv("statistical_analysis/shadow_length_vs_transcript_count/results/upstream_exon_shadow_vs_first_exon_tx_count.csv", index=False)
