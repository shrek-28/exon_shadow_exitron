...
for file in files:
    ...
    df['exon_length'] = df['last_exon_end'] - df['first_exon_start']

    df = df[['exon_length', 'upstream_len_in_scfr']].apply(
    ...
    x = df['exon_length'].values
    y = df['upstream_len_in_scfr'].values
...
results_df.to_csv(
    "statistical_analysis/exon_length_vs_shadow/results/"
    "upstream_exon_shadow_vs_exon_length_correlations.csv",
    index=False
)
