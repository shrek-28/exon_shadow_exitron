...
for file in files:
    ...
    df['scfr_length'] = df['end'] - df['start']

    df = df[['scfr_length', 'upstream_len_in_scfr']].apply(
...
results_df.to_csv(
    "statistical_analysis/scfr_length_vs_shadow_length/results/"
    "upstream_exon_shadow_vs_scfr_length_correlations.csv",
    index=False
)
